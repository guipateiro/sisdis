           ...
           case test: 
                ...
                do {
                     ...
                     if (status(processo[processo[token].teste].id) != 0){
                        ...
                        int j = (processo[token].teste + 1) % N;
                        printf("Processo %d: recebendo status dos processos {", token);
                        while (j != token){
                           if (processo[processo[token].teste].state[j] != -1){
                              processo[token].state[j] = processo[processo[token].teste].state[j];
                              printf("%d ", j);
                           }
                           j = (j + 1) % N;
                        }
